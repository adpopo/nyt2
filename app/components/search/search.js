...
class Search extends React.Component {
	...
	// sanitize date input from users
	validateDate(dt) {
	...
	handleSubmit(event) {
		...
		if (document.getElementById("topic").value.trim().length == 0)
		{
			...
			newTerm.errors.topic.msg = "Topic cannot be left left blank.";
		}
		if (!this.validateDate(beginDate)){
			newTerm.errors.beginDate.status = true;
			newTerm.errors.beginDate.msg = "Date formatting should be MM/DD/YYYY.";
		}
		else if (!this.checkBeginBeforeEndDate(formattedBeginDate,formattedEndDate)){
			newTerm.errors.beginDate.status = true;
			newTerm.errors.beginDate.msg = "Time travel isn't possible, check your dates";
		}
		if (!this.validateDate(endDate)){
			newTerm.errors.endDate.status = true;
			newTerm.errors.endDate.msg = "Date formatting should be MM/DD/YYYY.";
		}
		else if (!this.checkBeginBeforeEndDate(formattedBeginDate,formattedEndDate)){
			newTerm.errors.endDate.status = true;
			newTerm.errors.endDate.msg = "Time travel isn't possible, check your dates";
		}
